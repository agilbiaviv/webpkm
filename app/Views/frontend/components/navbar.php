<nav id="navbar" class="fixed top-0 left-0 w-full backdrop-blur-sm z-50 shadow dark:shadow-[0_2px_4px_rgba(255,255,255,0.06)] transition-transform duration-300 bg-white/90 text-gray-900 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 ease-in-out will-change-transform">
                ...
                <?php foreach ($menus as $menu): ?>
                    <?php if (!empty($menu['children'])): ?>
                        <div class="relative group hidden md:block">
                            <button class="flex items-center text-gray-700 dark:text-gray-300 hover:text-orange-500 dark:hover:text-orange-400 cursor-pointer">
                                <?= esc($menu['name']) ?>
        ...
        <div class="flex flex-col space-y-2">
            ...
            { ?>
                <?php foreach ($menus as $menu): ?>
                    <?php if (!empty($menu['children'])): ?>
                        <div>
                            <button type="button" class="w-full text-left py-2 px-4 flex items-center justify-between text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400"
                    ...
                    <?php else: ?>
                        <a href="<?= getMenuUrl($menu) ?>" class="block text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 py-2 pl-4">
                            <?= esc($menu['name']) ?>
                        </a>
                    <?php endif; ?>
                <?php endforeach; ?>
